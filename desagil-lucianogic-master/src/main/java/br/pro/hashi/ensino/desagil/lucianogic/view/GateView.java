...
public class GateView extends FixedPanel implements ItemListener {
	...
	public GateView(Gate gate) {
		...
		for(int i = 0; i < outSize; i++) { //criar mais de uma outBoxes
		...
		if(outSize == 1) {
			...
			for(int i = 0; i < outSize; i++) {
				add(outBoxes[i], 184, ((i + 1) * 58), 20, 20);
				if (i>0) {
					add(outBoxes[i], 184, ((i + 1) * 50), 20, 20); // nota: jeito muito feio de se fazer isso
